            ...
            <div className="mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row-reverse justify-between gap-8 sm:gap-0 items-center">
                    <div className='flex flex-col gap-2 items-center justify-center  '>
                        ...
                        <div className="flex flex-col gap-2  items-center justify-center">
                            <p className=' text-center text-sm   '> {`${language === 'ar' ? '   عبقرينو منصة سعودية مرخصة ' : ' Abqarinno Saudi platform licensed '} `} </p>
                            <p className='text-center text-sm  '> {`${language === 'ar' ? '(1009152474)  بموجب السجل التجاري' : 'by the commercial register (1009152474) '} `} </p>
                            <p className='text-center text-sm mt-2  '> {`${language === 'ar' ? '+966 53 847 رقم الواتساب : 4025' : 'Whatsapp : +966 53 847 4025'} `} </p>
                        </div>
                    </div>
                    ...
                    <div className='mt-10 md:mt-0 mb-10'>
                        <h1 className="text-base font-bold text-center text-blue-light mb-6"> {`${language === 'ar' ? 'تابعنا' : 'Follow Us'} `}</h1>
